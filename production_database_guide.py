...
def diagnose_database_setup():
    ...
    print(f"\n📡 Production API:")
    try:
        response = requests.get("https://realty-login.preview.emergentagent.com/api/apartments", 
                              params={"limit": 5})
        
        if response.status_code == 200:
            data = response.json()
            prod_count = data.get('total', 0)
            print(f"   API endpoint: https://realty-login.preview.emergentagent.com/api")
            print(f"   Total apartments: {prod_count}")
            
            # Check for the specific problem
            cpw_response = requests.get("https://realty-login.preview.emergentagent.com/api/apartments",
...
def create_deployment_instructions():
    ...
    print(f"   curl 'https://realty-login.preview.emergentagent.com/api/apartments?search=Central+Park+West'")
